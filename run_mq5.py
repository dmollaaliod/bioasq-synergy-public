...
with open('neural_tuned_rd4/rd4_neural_tuned_notrec0.5.tsv') as f:
...
#        if q['answerReady']:
        if True:
#            input_sentences = [s['text'] for s in snippets if (s['text'] not in gold_snippet_negatives) and (len(word_tokenize(s['text'])) <= 40)]
...
with open('round4_mq5.json','w') as f:
